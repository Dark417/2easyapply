# Eve — Design Product Document

> **Eve** is a Chrome extension that acts as a personal co-pilot when applying for jobs on LinkedIn. It lives as a floating popup on the screen, watches the user's Easy Apply flow, reads the form data step by step, and displays it in a clean table inside its panel.

---

## Vision

Eve is the ultimate LinkedIn Easy Apply assistant. It:
...
| 260314-R69 | In Eve popup Search delays, each delay must appear once by name with one low/high line, and auto mode must never freeze page interaction due modal focus lock | ✅ Done |
| 260313-R68 | Clarify menu/popup/tab vocabulary in agents rules, move version from Eve floating popup to extension menu, show current manual/auto mode by toggle in menu, keep dark/light text readable, move Search action to top of popup Search tab, and replace old ms delay controls with readable per-action low/high ranges in seconds (with hover explanations) | ✅ Done |
| 260313-R67 | Improve dark theme readability in Eve UI, simplify popup controls, add sun/moon dark-theme icon in popup header, remove popup version text, compact location/search fields, and always grey applied jobs on the current page in both manual and auto modes | ✅ Done |
| 260313-R66 | Popup menu moves dark theme toggle into the top-right header, stacks `Enable Eve` and `Apply Mode` toggles vertically in one row, and Eve floating panel must always restore on LinkedIn job pages even if a prior minimized flag was persisted | ✅ Done |
| 260308-R65 | Add canonical regex mappings for `*work*startup*` = `Yes`, `*Are you located in*` = `No`, and fully reset Eve Apply state when the modal closes so the next `Apply` click restarts cleanly | ✅ Done |
| 260308-R64 | Add canonical regex mappings for `Have you worked with*` = `Yes` and `*U.S. Citizen*` = `No`, and preserve the last seen step snapshot while resetting Eve Apply state after a mid-flow Easy Apply exit | ✅ Done |
| 260308-R63 | Add a canonical regex mapping for `Are you comfortable*` and seed the shared answer as `Yes` | ✅ Done |
| 260308-R62 | If Eve pauses on missing required fields, then after the user fills them manually and clicks `Next`, Eve should automatically resume the apply loop | ✅ Done |
| 260308-R61 | Clicking LinkedIn's visible `Easy Apply` button should reliably trigger Eve auto-apply even when LinkedIn wraps the click target in nested elements or composed/shadow event paths | ✅ Done |
...
| 260308-R59 | Eve popup/menu should be taller and avoid internal scrolling so all controls stay visible at once | ✅ Done |
...
| 260308-R57 | When Eve auto-fills a text field and LinkedIn opens an autocomplete or dropdown, Eve must pick the matching option before continuing | ✅ Done |
...
| 260308-R55 | Eve UI must show the current extension version in the floating panel too, and the manifest version must increment after each batch edit | ✅ Done |
...
| 260308-R53 | Clicking LinkedIn's own `Easy Apply` button should also trigger Eve auto-apply, show company and role in Eve, and continue the same automation loop | ✅ Done |
| 260308-R52 | Eve search setup on a fresh LinkedIn tab must fill role and location, open `All filters`, toggle `Easy Apply` on, and click `Show results` | ✅ Done |
| 260308-R51 | `regex: *keyword*` means map any question containing that keyword to a shared canonical Eve label and always reuse the same stored value | ✅ Done |
...
| 260308-R48 | Eve does not show or add the `Mark job as a top choice` step to its visible Step history | ✅ Done |
...
| 260308-R38 | Make minimized Eve smaller, still movable, and show a compact `A` icon | ✅ Done |
| 260308-R37 | Move theme switching from the floating Eve panel into the extension popup menu | ✅ Done |
...
| 260307-R34 | Create `design/task1-auto-flow.md` and add `lead-product` / `refine-task` workflow refinement instructions | ✅ Done |
...
| 260307-R32 | Move Eve theme switching into a header menu instead of a dedicated visible toggle | ✅ Done |
| 260307-R31 | Eve floating panel supports a persistent light theme and a higher-contrast dark theme via a header toggle | ✅ Done |
| 260307-R30 | Add local Python `lk` automation script to bridge `/params` and trigger Eve auto-apply across LinkedIn job tabs | ✅ Done |
...
| 260307-R27 | Add Eve `Apply` tab with page-side auto apply trigger | ✅ Done |
| 260307-R26 | Add Eve `Search` tab before `Step` with saved search text, dropdown, ignore keywords, and background-tab LinkedIn search trigger | ✅ Done |
| 260307-R25 | Add Eve floating panel minimize button that collapses to a single lightning icon and restores on click | ✅ Done |
...
| # | Requirement | Status |
|---|-------------|--------|
| 260228-R23 | Both `linkedin.com/jobs/search` and `linkedin.com/jobs/search/` URLs activate Eve (substring match covers both) | ✅ Done |
...
| 260228-R14 | Live sync: when user types in Easy Apply empty field, Eve table updates in real time | ✅ Done |
...
| 260228-R11 | Wider Eve panel (440px), scrollable body, 50/50 two-column table with per-cell overflow scroll | ✅ Done |
| 260228-R10 | In each Easy Apply step, show all form field labels + values as a table inside Eve | ✅ Done |
| 260228-R9 | Shadow DOM piercing: modal lives inside `#preact-border-shadow-host` shadow root | ✅ Done |
| 260228-R8 | Legacy extension popup directory rename completed | ✅ Done |
| 260228-R7 | Clear "Applying" text when modal is canceled or submitted | ✅ Done |
| 260228-R6 | Show "Applying: [step heading]" in Eve when Easy Apply modal is open | ✅ Done |
...
| 260228-R3 | Show a floating dark glassmorphism popup (Eve UI) on LinkedIn | ✅ Done |
| 260228-R2 | Use `logo.png` as the extension icon | ✅ Done |
| 260228-R1 | Extension is called "Eve" (nickname for easyapply plugin) | ✅ Done |
...
```
easyapply/
├── eve/               ← Chrome extension source
│   ├── manifest.json
│   ├── content.js      ← Core logic: shadow DOM, polling, field extraction
│   ├── background.js
│   ├── popup.html / popup.js
│   ├── ui.css          ← Eve's floating UI styles
│   └── logo.png
├── design/
│   ├── eve.md         ← This file. Design product doc.
...
- Sections:
  - **Header**: `⚡ Eve` + green pulse indicator
