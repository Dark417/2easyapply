...
(function () {
    ...
    function buildGH_PROFILE_FIELDS() {
        ...
        { label: /^school\b|^university\b|^college\b|(most recent|current) (school|university)|education institution/i, value: GH_PROFILE.school },
        { label: /^graduation date|graduation \(date|date of graduation/i, value: GH_PROFILE.graduationDate },
        { label: /^graduation year|year of graduation/i, value: GH_PROFILE.graduationYear },
        ...
        try {
            ...
            for (let round = 1; round <= MAX_FILL_ROUNDS; round += 1) {
                ...
                for (const radios of radioGroups.values()) {
                    ...
                    const picker = entry ? chooserPicker(entry) : null;
                    if (!picker) {
                        // Standing rule (user, 2026-07-28): a radio group offering exactly ONE real
                        // choice is not a choice — select it, bank match or not. Same reasoning as
                        // the single-option dropdown and the lone checkbox.
                        const singleIndex = pickSingletonIndex(optionLabels);
                        if (singleIndex >= 0) {
                            realClick(radios[singleIndex]);
                            await afterSelection();
                            filled.push(`${groupLabel} → ${optionLabels[singleIndex]} (only option)`);
                            continue;
                        }
