        ...
        item.addEventListener('click', (e) => {
            if (e.target.classList.contains('sl-del')) {
                ...
                chrome.storage.local.get(['eveParams'], (res) => {
                    const nextParams = gatherPopupParams(res.eveParams || {});
                    ...
                    chrome.runtime.sendMessage({ type: 'eve:save-params', params: nextParams }, (response) => {
        ...
        item.addEventListener('drop', (e) => {
            ...
            if (dragSrc && dragSrc !== item) {
                ...
                chrome.storage.local.get(['eveParams'], (res) => {
                    ...
                    chrome.runtime.sendMessage({ type: 'eve:save-params', params: nextParams }, () => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.storage.local.get(['settings', 'savedAnswers', 'eveParams', 'eveTheme', 'eveApplyMode', 'eveApplyStats'], (res) => {
        ...
        const applyMode = res.eveApplyMode === 'manual' ? 'manual' : 'auto';
        document.getElementById('applyModeToggle').checked = applyMode === 'auto';
        updateApplyModeLabel(applyMode);
        const applyStats = Object.assign({ auto: 0, manual: 0 }, res.eveApplyStats || {});
    ...
    chrome.storage.onChanged.addListener((changes, areaName) => {
        ...
        if (changes.eveTheme) {
            const isDark = changes.eveTheme.newValue === 'dark';
            document.getElementById('themeToggle').checked = isDark;
            document.body.classList.toggle('light-theme', !isDark);
            updateThemeIcon(isDark);
        }
        if (changes.eveApplyMode) {
            const applyMode = changes.eveApplyMode.newValue === 'manual' ? 'manual' : 'auto';
            document.getElementById('applyModeToggle').checked = applyMode === 'auto';
            updateApplyModeLabel(applyMode);
        }
        if (changes.eveApplyStats) {
            const applyStats = Object.assign({ auto: 0, manual: 0 }, changes.eveApplyStats.newValue || {});
            document.getElementById('auto-count').textContent = applyStats.auto;
            document.getElementById('manual-count').textContent = applyStats.manual;
        }
        if (changes.settings) {
    ...
    document.getElementById('applyModeToggle').addEventListener('change', function () {
        ...
        chrome.storage.local.set({ eveApplyMode: mode });
    });
    ...
    document.getElementById('runSearchBtn').addEventListener('click', () => {
        chrome.storage.local.get(['eveParams'], (res) => {
            chrome.runtime.sendMessage({ type: 'eve:open-search', params: gatherPopupParams(res.eveParams || {}) });
        });
    });
